...
void writeActions(int id,string Name){
	ofstream output("output.txt");
	output<<id<<" "<<Name;
}
void writeActions(int id){
	ofstream output("output.txt");
	output<<id;
}
...
bool addToActions(unordered_map<string,vector<string>> sentenceTags){
	...
	if(sentenceTags.find("VS")!=end){
		 ...
		 for(auto& it : sentenceTags["VS"]){
			 //Save faces
			 string sName;
			 if(it=="remember" || it=="save" || it="learn"){
			...
			if(it=="repeat"){
				cout<<"Repeat the sentence"<<cout;
				//Revisar que parte de la oracion que quiere que repitamos
				writeActions(5);
				return true;
			}
		 }
...
string readSentence(){
	//Loads the data from dictionary.txt into the hashmap
	ifstream dataFile("sentence.txt");
	if (!dataFile.is_open()) {
		 // The file hasn't been opened; take appropriate actions here.
		 throw  "File could not be opened F";
	 }
	 string sentence;
